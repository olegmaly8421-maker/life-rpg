...
export default function Auth() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <Card style={{ width: '100%', maxWidth: 370, padding: 26 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          ...
          <Input type="password" placeholder="Пароль (мін. 6 символів)" value={password}
            onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} />
          <Btn onClick={submit} disabled={loading} style={{ marginTop: 5, opacity: loading ? 0.6 : 1 }}>
            {loading ? '...' : mode === 'signin' ? 'Увійти' : 'Зареєструватись'}
          </Btn>
        </div>
        {msg && <div style={{ color: msg.includes('створено') ? '#22C55E' : '#EF4444', fontSize: 12, marginTop: 11, textAlign: 'center' }}>{msg}</div>}
