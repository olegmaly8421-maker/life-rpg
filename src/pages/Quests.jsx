        ...
        return (
          <div key={type}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {items.map((q) => (
                <Card key={q.id} style={{
                  background: q.done ? '#0D2818' : '#12121A',
                  border: `1px solid ${q.done ? '#22C55E44' : '#1E1E2E'}`, padding: '13px 15px',
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 11 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: q.done ? '#22C55E' : '#E2E8F0', fontWeight: 600, fontSize: 14 }}>
                        {q.done ? '✓ ' : ''}{q.title}
                      </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexShrink: 0 }}>
                      ...
                      {q.total != null ? (
                        ...
                        <button onClick={() => toggleDone(q)} style={{
                          ...btn, padding: '5px 11px',
                          color: q.done ? '#22C55E' : '#9CA3AF',
                          border: `1px solid ${q.done ? '#22C55E' : '#374151'}`,
                        }}>{q.done ? 'Done' : 'Mark'}</button>
                      )}
                    </div>
                  </div>
                </Card>
